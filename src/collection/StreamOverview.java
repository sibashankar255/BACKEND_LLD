...
//        long output = salaryList.stream().filter((Integer sal) -> sal>3000).count();
        ...
        filter(Predicate<T> predicate): Filters the elements based on a given predicate and returns a new stream containing only the elements that match the predicate.
        map(Function<T, R> mapper): Transforms each element of the stream using the provided function and returns a new stream of transformed elements.
        ...
        sorted(): Sorts the elements in natural order. You can also provide a custom Comparator as an argument.
        peek(Consumer<T> action): Applies an action to each element as they are consumed from the stream, often used for debugging or logging.
        ...
        forEach(Consumer<T> action): Applies the provided action to each element in the stream. This is useful for performing an action on each element, such as printing or processing.
        toArray(): Collects the elements in the stream into an array and returns the array.
        reduce(BinaryOperator<T> accumulator): Reduces the elements in the stream to a single value by applying the provided binary operator.
        collect(Collector<T, A, R> collector): Performs a mutable reduction operation on the elements using a Collector.
        ...
        anyMatch(Predicate<T> predicate): Checks if any elements in the stream match the provided predicate. Returns a boolean.
        allMatch(Predicate<T> predicate): Checks if all elements in the stream match the provided predicate. Returns a boolean.
        noneMatch(Predicate<T> predicate): Checks if none of the elements in the stream match the provided predicate. Returns a boolean.
        ...
        toMap(Function<T, K> keyMapper, Function<T, U> valueMapper): Collects the elements in the stream into a Map using the provided key and value mappers.
        toCollection(Supplier<C> collectionFactory): Collects the elements into a custom collection provided by the collectionFactory.
        summarizingInt(ToIntFunction<T> mapper): Returns a summary statistics object for int-valued elements (useful for statistics like count, sum, min, max, and average).
